...
type Product = {
    ...
    discount?: string;           // Optional for legacy data
    image: string;
    category: string;
    filterSlug?: string;
};
...
export default function ProductCard({
    ...
    // Dynamic discount calculation (preferred)
    const discountInfo = product.originalPrice && product.originalPrice > product.price
        ? {
            percent: Math.round(((product.originalPrice - product.price) / product.originalPrice) * 100),
            label: `SAVE ${Math.round(((product.originalPrice - product.price) / product.originalPrice) * 100)}%`
        }
        : null;

    const finalDiscountLabel = discountInfo?.label || product.discount;
    ...
    return (
        ...
        >
            <div className={`relative bg-[#f9f9f9] overflow-hidden ${
                ...
                {/* Discount Badge - only shown when enabled */}
                {showDiscount && finalDiscountLabel && (
